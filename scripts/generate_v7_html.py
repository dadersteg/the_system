...
def generate_v7_html():
    ...
    with open('docs/Bookmarks/V7_Empty_LOS_Bookmarks.html', 'w', encoding='utf-8') as f:
        ...
        def write_node(node, indent_level):
            ...
            if not children:
                # Leaf node: Add a dummy bookmark so Chrome doesn't prune this folder!
                f.write(f'{indent}    <DT><A HREF="https://www.google.com" ADD_DATE="{timestamp}">[Delete Me] Dummy Bookmark</A>\\n')
            else:
