...
def extract_bluf(markdown_text):
    ...
    for line in lines:
        ...
        if line_strip.startswith('**Concept:**'):
            ...
            state = 'edge' # use this as edge if no edge
            ...
            # Probably variable lists
            if state == 'concept' or state == 'edge':
                pass # usually these are part of Variables to Track, let's stop state
            state = None
            continue
    ...
    # Cleaning up any lingering bold tags inside edge/concept
    ...
    if takeaway_source:
        takeaway = takeaway_source.split('. ')[0]
...
if __name__ == '__main__':
    ...
    print(json.dumps({k: out_data[k] for k in list(out_data.keys())[:2]}, indent=4))
    print("...")
    print(json.dumps({k: out_data[k] for k in list(out_data.keys())[-3:]}, indent=4))
