...
function resolveFolderFromTaxonomy(concatPath, taxonomy) {
  ...
  for (let i = 0; i < folderNames.length; i++) {
      ...
      while (files.hasNext()) {
        ...
        if (!file.isTrashed() && file.getMimeType() === "application/vnd.google-apps.shortcut") {
          try {
            ...
            found = DriveApp.getFolderById(targetId);
            break;
          } catch(err) {
            console.warn(`Could not resolve shortcut target for '${part}': ${err.message}`);
          }
        }
      }
    }

    // C. Case-insensitive fallback among direct child folders
    if (!found) {
      const allSubs = currentFolder.getFolders();
      while (allSubs.hasNext()) {
        const f = allSubs.next();
        if (!f.isTrashed() && f.getName().toLowerCase().trim() === part.toLowerCase()) {
          found = f;
          break;
        }
      }
    ...
    // D. Case-insensitive fallback among shortcuts
    if (!found) {
      const allFiles = currentFolder.getFiles();
      while (allFiles.hasNext()) {
        const file = allFiles.next();
        if (!file.isTrashed() && file.getMimeType() === "application/vnd.google-apps.shortcut" && file.getName().toLowerCase().trim() === part.toLowerCase()) {
          try {
            const targetId = file.getTargetId();
            found = DriveApp.getFolderById(targetId);
            break;
