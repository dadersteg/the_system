...
function _analyzePhotoWithGemini(blob, msgContext) {
  ...
  let parsed = callGemini(parts, modelName, "You are an expert image analyzer.", null);
  if (!parsed || parsed.error) {
    const fallback = SYSTEM_CONFIG.SECRETS.GEMINI_FALLBACK_LITE || "gemini-flash-lite-latest";
    console.warn(`Photo Sync analysis failed on ${modelName}, attempting fallback to ${fallback}...`);
    parsed = callGemini(parts, fallback, "You are an expert image analyzer.", null);
  }
