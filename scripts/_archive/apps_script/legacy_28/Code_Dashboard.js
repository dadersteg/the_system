...
function isAuthorized(e) {
  ...
  const allowedEmails = [
    ...
    (getEnvProp("CE_EMAIL") || "daniel@martens-adersteg.com")
  ];
...
function parseExecutionPlan(md) {
  const data = { bluf: "", frogs: { ce: [], personal: [] }, top3: { ce: [], personal: [] }, rest: { ce: [], personal: [] }, alerts: [], triage: "" };
  if (typeof md !== "string") {
  ...
  try {
    ...
    for (let line of lines) {
      ...
      if (line.match(/^\*\*CE:\*\*/i) || line.match(/^\*\*🎯 CE:\*\*/i) || line.match(/^\*\*CE \(Work\):\*\*/i) || line.match(/^\*\*🎯 CE \(Work\):\*\*/i)) {
        currentCategory = "ce";
        continue;
      }
