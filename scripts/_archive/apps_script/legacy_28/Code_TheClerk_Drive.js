...
`;
    ...
    let result = callGemini(parts, currentModel, rules + taskInstruction, null, false);
    ...
    if (!result || result.error) {
      ...
      if (currentModel !== liteFallback) {
        ...
        result = callGemini(parts, liteFallback, rules + taskInstruction, null, false);
      }
    }
    if (!result || result.error) {
      const proFallback = SYSTEM_CONFIG.SECRETS.GEMINI_FALLBACK_PRO || "gemini-pro-latest";
      if (currentModel !== proFallback) {
        console.warn(`The Clerk Drive fallback failed, attempting Pro Fallback: ${proFallback}...`);
        result = callGemini(parts, proFallback, rules + taskInstruction, null, false);
      }
    }
    if (result && !result.error) {
        let items = null;
