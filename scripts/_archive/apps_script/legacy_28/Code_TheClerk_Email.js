...
function executeTriageEngine(searchQuery, searchLimit, isRetro, configPayload) {
  ...
  const personalGoalsStr = configPayload ? configPayload.personalGoalsStr : (IS_CE_ENV ? "" : getSafeDocText(SYSTEM_CONFIG.DOCS.PERSONAL_GOALS_FILE_ID));
  ...
  for (let index = 0; index < threads.length; index++) {
    ...
    const logData = _processSingleEmailThread(
      ...
      isRetro, currentModel, personalGoalsStr, workGoalsStr, blufPrompt
    );
...
4. CRITICAL: Do NOT output any URLs, web links, or markdown link formatting. Output pure plain text only.`;
  ...
  let parsed = callGemini(parts, modelName, "You are a helpful email clerk assistant.", schema);
  
  // Resilient Ingestion Fallback: 3.7 Flash -> Flash Lite -> Pro
  if (!parsed || parsed.error) {
    const liteFallback = SYSTEM_CONFIG.SECRETS.GEMINI_FALLBACK_LITE || "gemini-flash-lite-latest";
    if (modelName !== liteFallback) {
      ...
      parsed = callGemini(parts, liteFallback, "You are a helpful email clerk assistant.", schema);
    }
  }
  if (!parsed || parsed.error) {
    const proFallback = SYSTEM_CONFIG.SECRETS.GEMINI_FALLBACK_PRO || "gemini-pro-latest";
    if (modelName !== proFallback) {
      console.warn(`The Clerk Email fallback failed, attempting Pro Fallback: ${proFallback}...`);
      parsed = callGemini(parts, proFallback, "You are a helpful email clerk assistant.", schema);
    }
  }
