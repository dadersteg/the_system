                ...
                if TARGET_CONVO:
                    ...
                    for url, title in ws_urls:
                        if target_lower in title.lower():
                            return url
                    # If target is set but not found on this port, check the next port
                    continue
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'approve':
        ...
        res = await answer_question(num)
        msg = update.callback_query.message if update.callback_query else update.message
        if res == "Success":
            await msg.reply_text(f"✅ Selected option {num}")
            ws_url = await get_ws_url_for_target()
            if ws_url:
                asyncio.create_task(monitor_response(update, ws_url))
        else:
            await msg.reply_text(f"⚠️ {res}")
            ...
            (function() {{
                ...
                if (submitBtn) {{
                    ...
                    if (!isNaN(num) && num > 0 && num <= options.length) {{
                        const radio = options[num - 1];
                        const targetBtn = radio.closest('label') || radio.parentElement || radio;
                        targetBtn.click();
                        submitBtn.click();
                        return "Submitted";
                    }}
...
async def get_overview_data():
    ...
    if not pages:
        return "⚠️ Failed to connect to CDP.", None

    ws_urls = [(p.get("webSocketDebuggerUrl"), p.get("title", ""), p.get("url", "")) 
               for p in pages 
               if (p.get("url", "").split("?")[0].endswith("workbench.html") or p.get("type") == "page") and "webSocketDebuggerUrl" in p]
    ...
    if TARGET_CONVO:
        ...
        if not active_url:
            return f"⚠️ Target workspace '{TARGET_CONVO}' not found among open tabs.", None
            ...
            (function() {{
                ...
                if (!isNaN(num) && num > 0) {{
                    ...
                    if (!targetBtn) {{
                        ...
                        if (options.length >= num) {{
                            const radio = options[num - 1];
                            targetBtn = radio.closest('label') || radio.parentElement || radio;
                        }}
                    }}
                }}
                ...
                if (doSubmit) {{
                    return new Promise(resolve => {{
                        ...
                        const checkSubmit = () => {{
                            attempts++;
                            if (submitBtnFound && !submitBtnFound.disabled && submitBtnFound.innerText !== "Submit") {{
                            ...
                            let submitBtn = allBtns.find(b => {{
                                const t = (b.innerText || b.value || "").trim().toLowerCase();
                                return t.includes("submit") || t.includes("confirm") || t.includes("send") || t.includes("proceed");
                            }});
...
async def unread_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ws_url = await get_ws_url_for_target()
    if not ws_url:
        await update.message.reply_text("⚠️ Bridge offline or no active Antigravity tab found.")
        return
    ...
    try:
        async with websockets.connect(ws_url, open_timeout=2.0) as ws:
            script = f"""
            (function() {{
                const unreads = Array.from(document.querySelectorAll('.animate-unread-ping'));
                if (unreads.length === 0) return "No unread conversations found.";
                
                let index = 0;
                let targetStr = "{target}";
                if (targetStr === "last") {{
                    index = unreads.length - 1;
                }} else {{
                    let parsed = parseInt(targetStr, 10);
                    if (!isNaN(parsed) && parsed > 0 && parsed <= unreads.length) {{
                        index = parsed - 1;
                    }} else {{
                        return "Invalid number. You have " + unreads.length + " unread conversations.";
                    }}
                }}
                
                const unread = unreads[index];
                const btn = unread.closest('a') || unread.closest('button') || unread.parentElement.parentElement;
                if (btn) {{
                    btn.click();
                    return "Success";
                }}
                return "Could not click the conversation.";
            }})();
            """
            await ws.send(json.dumps({
                "id": 1,
            ...
            if val == "Success":
                ...
                await update.message.reply_text(f"⚠️ {val}")
    except Exception as e:
