...
def main():
    ...
    print(f"Found {len(redundancies)} items to trash.")
    success_count = 0
    fail_count = 0

    for item in redundancies:
        try:
            # THIS IS A SAFE DELETE. It sets 'trashed': True which moves to the bin for 30 days.
            # Permanent deletion would use drive_service.files().delete(fileId=...)
            drive_service.files().update(fileId=item['id'], body={'trashed': True}).execute()
            print(f"Moved to trash: {item['name']} ({item['id']})")
            success_count += 1
        except Exception as e:
            print(f"Failed to trash {item['name']} ({item['id']}): {e}")
            fail_count += 1

    print(f"\nFinished trashing. Success: {success_count}, Failed: {fail_count}")
