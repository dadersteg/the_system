...
load_dotenv('/Users/daniel/Developer/the_system/.env') # Loads variables from the .env file
...
def get_access_token():
    ...
    clasprc_path = os.environ.get('CLASPRC_PATH', os.path.expanduser('~/.clasprc.json'))
...
async def handler(event):
    ...
    if not sender or getattr(sender, 'id', None) != 8971429996:
        return
    ...
    # --- SMS RELAY DETECTION ---
    # When MacroDroid sends SMS via the Telegram bot (id 8971429996),
    # the message starts with "📱 [SMS] <number>". Use "SMS: <number>"
    # as the thread name so it appears correctly in Gmail.
    is_sms_relay = False
    if sender and getattr(sender, 'bot', False) and sender.id == 8971429996:
