/**
 * @file 09_WorkspaceTaxonomy.js
 * @description Master LOS Taxonomy Parser, Drive Folder Tree Generation, [DONE] Reset Engine & Gmail Label Management.
 */

// ==========================================
// SECTION 1: MASTER TAXONOMY PARSER & DRIVE TREE
// ==========================================
...
function syncTaxonomyToSheet() {
  ...
  if (isCe) {
    // Parse using CE flat taxonomy logic
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (currentSection === 'core') {
        ...
        if (mSub && currentL1Code) {
          ...
          let label = `${currentL1Code} ${currentL1Name}/${folderName}`;
          let path = `${currentL1Code} ${currentL1Name}/${folderName}`;
          if (currentL1Code === "00" || currentL1Code === "99") {
            if (folderName.startsWith("00 ") || folderName.startsWith("99 ")) {
              label = folderName;
              path = folderName;
            }
          }
  ...
  // Adjust labels for CE account context 
  // (Obsolete logic for stripping '02 Work/01 Employment/01 Playmetech/' removed as CE is now flat)
    ...
    for (let j = 0; j < parts.length; j++) {
      ...
      try {
        ...
        while (folders.hasNext()) {
          ...
          if (!f.isTrashed()) {
            nextFolderId = f.getId();
            break;
          }
        }

        // 2. Shortcut exact match
        if (!nextFolderId) {
          const files = currentFolder.getFilesByName(part);
          while (files.hasNext()) {
            const file = files.next();
            if (!file.isTrashed() && file.getMimeType() === "application/vnd.google-apps.shortcut") {
              const target = getShortcutTargetFolder(file);
              nextFolderId = target ? target.getId() : file.getId();
              break;
            }
          }
        }

        // 3. Prefix / fuzzy folder match
        if (!nextFolderId) {
          const allSubs = currentFolder.getFolders();
          while (allSubs.hasNext()) {
            const f = allSubs.next();
            if (!f.isTrashed() && matchesFolderName(f.getName(), part)) {
              nextFolderId = f.getId();
              break;
            }
          }
        ...
        // 4. Prefix / fuzzy shortcut match
        if (!nextFolderId) {
          const allFiles = currentFolder.getFiles();
          while (allFiles.hasNext()) {
            const file = allFiles.next();
            if (!file.isTrashed() && file.getMimeType() === "application/vnd.google-apps.shortcut" && matchesFolderName(file.getName(), part)) {
              const target = getShortcutTargetFolder(file);
              nextFolderId = target ? target.getId() : file.getId();
              break;
            }
  ...
  const taxPrefix = isCe ? "CE LOS_Taxonomy" : "LOS_Taxonomy";
  ...
  } else if (SYSTEM_CONFIG && SYSTEM_CONFIG.ROOTS && SYSTEM_CONFIG.ROOTS.WORKSPACE_FOLDER_ID) {
    try {
      parentFolder = DriveApp.getFolderById(SYSTEM_CONFIG.ROOTS.WORKSPACE_FOLDER_ID);
    } catch(e) {
      console.warn("Could not get WORKSPACE_FOLDER_ID: " + e.message);
    }
  }
  if (!parentFolder) {
  ...
  let jsonFile = null;
  if (SYSTEM_CONFIG && SYSTEM_CONFIG.DOCS && SYSTEM_CONFIG.DOCS.TAXONOMY_JSON_ID) {
    try {
      jsonFile = DriveApp.getFileById(SYSTEM_CONFIG.DOCS.TAXONOMY_JSON_ID);
      jsonFile.setContent(jsonBlob.getDataAsString());
    } catch(e) {
      jsonFile = null;
    }
  }
...
function syncDriveFoldersFromTaxonomy() {
  ...
  try {
    ...
    const isCe = IS_CE_ENV;
    const BATCH_SIZE = 10;
    const TARGET_DEPTH = 6; // Increased from 4 to support CE LOS nested structures (e.g. 5-level Bridge)
...
function resolveFolderFromTaxonomy(concatPath, taxonomy) {
  if (!concatPath || concatPath === "Unknown") return null;

  // 1. Clean & normalize input path
  let cleanInput = concatPath.toString().trim()
    .replace(/^my drive[\/>\s]+/i, "")
    .replace(/^root[\/>\s]+/i, "");

  let normInput = cleanInput.replace(/\s*>\s*/g, "/").replace(/^\/+|\/+$/g, "");
  if (!normInput) return null;

  let drivePath = null;
  let item = null;

  // 2. Try to match against taxonomy list if provided with symmetric normalization
  if (taxonomy && Array.isArray(taxonomy)) {
    item = taxonomy.find(t => {
      if (!t) return false;
      const cp = (t["Concat (Path)"] || "").trim();
      const dp = (t["Drive Path"] || "").trim();
      const normCp = cp.replace(/\s*>\s*/g, "/").replace(/^\/+|\/+$/g, "");
      const normDp = dp.replace(/\s*>\s*/g, "/").replace(/^\/+|\/+$/g, "");

      return cp === cleanInput || 
             normCp === normInput || 
             normDp === normInput ||
             normDp.endsWith(normInput) ||
             normCp.toLowerCase() === normInput.toLowerCase() ||
             normDp.toLowerCase() === normInput.toLowerCase();
    });

    // 2a. Direct Drive ID O(1) Fast-Path
    if (item && item["Drive ID"] && item["Drive ID"].trim() !== "") {
      try {
        const directFolder = DriveApp.getFolderById(item["Drive ID"].trim());
        if (directFolder && !directFolder.isTrashed()) return directFolder;
      } catch(err) {
        // If the ID was a shortcut file instead of folder, attempt shortcut resolution
        try {
          const directFile = DriveApp.getFileById(item["Drive ID"].trim());
          if (directFile && !directFile.isTrashed()) {
            const targetFolder = getShortcutTargetFolder(directFile);
            if (targetFolder) return targetFolder;
          }
        } catch(fileErr) {}
        console.warn(`Direct Drive ID lookup failed for '${item["Drive ID"]}': ${err.message}`);
      }
    }

    if (item && item["Drive Path"]) {
      drivePath = item["Drive Path"].trim();
    }
  }

  // 3. Fallback to normInput directly as path + L1 Domain Auto-Prefix Fallback
  if (!drivePath) {
    drivePath = normInput;
    if (!drivePath.match(/^(01 00 00 Private|02 00 00 Work|03 00 00 Studies|99 00 00 Archive)/i)) {
      if (drivePath.startsWith("01")) drivePath = "01 00 00 Private/" + drivePath;
      else if (drivePath.startsWith("02")) drivePath = "02 00 00 Work/" + drivePath;
      else if (drivePath.startsWith("03")) drivePath = "03 00 00 Studies/" + drivePath;
      else if (drivePath.startsWith("99")) drivePath = "99 00 00 Archive/" + drivePath;
    }
  }

  // 4. Construct folder hierarchy
  const folderNames = drivePath.split("/")
    .map(s => s.trim())
    .filter(s => s && s.toLowerCase() !== "my drive" && s.toLowerCase() !== "root");

  if (folderNames.length === 0) return "VIRTUAL_LABEL";

  // 5. Traverse down from Root
  ...
  for (let i = 0; i < folderNames.length; i++) {
    ...
    // A. Check direct folder (exact match)
    ...
    // B. Check folder shortcut (exact match)
    if (!found) {
      const files = currentFolder.getFilesByName(part);
      while (files.hasNext()) {
        const file = files.next();
    ...
    // C. Case-insensitive & prefix/fuzzy fallback among direct child folders
    if (!found) {
      const allSubs = currentFolder.getFolders();
      while (allSubs.hasNext()) {
        const f = allSubs.next();
    ...
    // D. Case-insensitive & prefix/fuzzy fallback among shortcuts
    if (!found) {
      const allFiles = currentFolder.getFiles();
      while (allFiles.hasNext()) {
        const file = allFiles.next();
    ...
    if (!found) {
      console.warn(`Traverse failed: Could not find active folder or shortcut '${part}' inside '${currentFolder.getName()}'`);
      return null;
    }
...
/**
 * Safely resolves the target Drive folder for a shortcut in Apps Script.
 * @param {DriveApp.File} file - The shortcut file object.
 * @returns {DriveApp.Folder|null} The resolved target folder or null.
 */
function getShortcutTargetFolder(file) {
  if (!file) return null;
  // 1. Try Advanced Drive Service if available
  if (typeof Drive !== 'undefined' && Drive.Files && Drive.Files.get) {
    try {
      ...
      if (meta && meta.shortcutDetails && meta.shortcutDetails.targetId) {
        const targetFolder = DriveApp.getFolderById(meta.shortcutDetails.targetId);
        if (targetFolder && !targetFolder.isTrashed()) return targetFolder;
      }
    } catch (e) {}
  }
  // 2. Try file.getTargetId if supported
  ...
  // 3. Fallback: try retrieving directly by ID if folder
  try {
    const direct = DriveApp.getFolderById(file.getId());
    if (direct && !direct.isTrashed()) return direct;
  } catch (e) {}
  return null;
}

/**
 * Robust folder name matcher supporting exact, case-insensitive, prefix, and suffix matches.
 * e.g. "01 Admin" matches "01 Admin (Humanoid) (Private)"
 * @param {string} actualName - The name of the physical folder in Google Drive.
 * @param {string} queryName - The taxonomy segment being searched for.
 * @returns {boolean} True if matching.
 */
function matchesFolderName(actualName, queryName) {
  if (!actualName || !queryName) return false;
  const a = actualName.toLowerCase().trim();
  const q = queryName.toLowerCase().trim();
  if (a === q) return true;
  if (a.startsWith(q + " ") || a.startsWith(q + " (") || a.startsWith(q + " -") || a.startsWith(q + "_")) return true;
  
  // Normalization for punctuation / pluralization (e.g. Contract vs Contracts)
  const normA = a.replace(/s\b/g, "").replace(/[,\s]+/g, " ").trim();
  const normQ = q.replace(/s\b/g, "").replace(/[,\s]+/g, " ").trim();
  if (normA === normQ || normA.startsWith(normQ)) return true;
  return false;
}
...
/**
 * Utility function to force sync the Humanoid folders and ensure they are created immediately.
 * This runs the taxonomy sync to update LOS_Taxonomy.json based on the Markdown,
 * then iterates through to ensure all Humanoid physical Drive folders exist.
 */
function forceSyncHumanoidFolders() {
  console.log("Starting forced sync for Humanoid folders...");
  
  // 1. Ensure the taxonomy JSON is up to date based on the Markdown
  syncTaxonomyToSheet();
  
  // 2. Load the updated taxonomy JSON
  const fileId = SYSTEM_CONFIG.DOCS.TAXONOMY_JSON_ID;
  const taxonomyFile = DriveApp.getFileById(fileId);
  const taxonomy = JSON.parse(taxonomyFile.getBlob().getDataAsString());
  ...
  // 3. Scan for Humanoid paths and force create them
  for (let i = 0; i < taxonomy.length; i++) {
    const item = taxonomy[i];
    if (item && item["Drive Path"] && item["Drive Path"].includes("Humanoid")) {
      const folderNames = item["Drive Path"].split("/").map(s => s.trim());
  ...
  console.log(`Completed forced sync. Created ${createdCount} missing folders for Humanoid.`);
}


// ==========================================
// SECTION 2: GMAIL LABEL LISTING & DEDUPLICATION
// ==========================================
...
function cleanAndCreateGmailLabels() {
  ...
  console.log(`Starting Gmail Labels cleanup and alignment for ${isCe ? "CE (Work)" : "Private"} profile...`);
  const labels = GmailApp.getUserLabels();
  ...
  const taxonomyFilename = isCe ? "CE LOS_Taxonomy.json" : "LOS_Taxonomy.json";
...
function migrateDuplicateGmailLabels() {
  ...
  console.log(`Starting migration of duplicate Gmail labels for ${isCe ? "CE (Work)" : "Private"} profile...`);
