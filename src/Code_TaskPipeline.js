...
/**
 * Main execution function to be scheduled every 15 minutes.
 */
function syncActionsToTasks() {
...
1. DEDUPLICATE: Compare "newActions" against "existingTasks". If a new action is completely covered by an existing task, DISCARD IT. Do not create duplicates.
2. CONSOLIDATE: If multiple new actions are just variations of the same underlying task, merge them into a single concrete action.
3. CATEGORIZE: For each valid new task, assign the most specific category. If a relevant category exists, you MUST use an exact string match from the "Concat (Path)" property found in the FULL LOS TAXONOMY below. If no category fits, output "N/A".
4. FORMAT: Titles MUST be structured as "[Action Verb] [Object]". Example: "Pay electricity bill".
5. METADATA: Estimate a "deadline" (Format: YYYY-MM-DD). If a deadline must exist but cannot be determined, output "TBC". If no deadline is set, output "None". If it is truly a task where a deadline is inapplicable, output "N/A". Estimate a "duration" (Format: e.g. "15m", "1h", or "N/A").
