/**
 * @file 02_TasksCore.js
 * @description Google Tasks API Integration, Conversation Ingestion, Task Schema Formatting, Completed Log Sync & Purge Routines.
 */

// ==========================================
// SECTION 1: CORE TASKS PIPELINE & GOVERNANCE
// ==========================================
 ...
 * - Context-aware categorization using the workspace taxonomy (LOS/CE LOS)
 */
...
const CONFIG = {
  ...
  includeHidden: false,    
  charLimit: 2000,         
  get spreadsheetId() { return SYSTEM_CONFIG.ROOTS.MASTER_SHEET_ID; },
  get targetGid() { return SYSTEM_CONFIG.SHEETS.TASK_REVIEW; },
 ...
 * CE (Work), and Private markdown task logs. The GAS functions remain intact so they 
...
function extractTasksWithConversationDetails() {
  ...
  const targetGidStr = (CONFIG.targetGid || "").toString();
  let sheet = ss.getSheets().find(s => s.getSheetId().toString() === targetGidStr);
  if (!sheet) {
    sheet = ss.getSheets().find(s => s.getName().includes("Google Tasks") || s.getName().includes("Tasks Log"));
  }
  
  const taskLists = fetchTaskLists();
  if (!taskLists) {
    console.error("fetchTaskLists returned null or empty. Aborting.");
    return;
  }
  ...
  if (sheet) {
    try {
      ...
      console.warn("Could not format header row: " + e.message);
    }
  } else {
    console.warn("Target sheet for Tasks Log not found. Skipping sheet export and continuing to Drive...");
  }
  ...
  tasks.forEach(item => {
    if (item.task && item.task.id) {
      taskTitleMap.set(item.task.id, item.task.title || "");
    }
  });
  ...
  if (sheet) {
    try {
      ...
      console.warn("exportResultsToSheet failed: " + errSheet.message);
    }
  }
 ...
 */
function buildTaskExportRow(task, taskList, emailInfo, exportTs, rowCounter, validPaths, allowedAliases, taskLists, taskTitleMap) {
  ...
  if (!isLOSValid) {
    ...
    if (titleParts.length >= 2) {
      systemComment = `Invalid ${IS_CE_ENV ? "CE LOS" : "LOS"} Path: Not found in taxonomy.`;
    } else {
  ...
  const parentTitle = (task.parent && taskTitleMap) ? taskTitleMap.get(task.parent) : "";
  const milestoneVal = extractMilestone(task.title, task.notes, parentTitle);
...
function getExportDescriptions() {
  const tax = IS_CE_ENV ? "CE LOS" : "LOS";
  return [
    ...
    "Linked system goal URN / ID", 
    "Resolved system goal title / name", 
...
function exportTasksToMarkdownDrive(results) {
  ...
  const fileName = IS_CE_ENV ? "Google Tasks (CE).md" : "Google Tasks (Private).md";
  ...
  try {
    ...
    if (fileId) {
      try {
        ...
        console.log(`Updated Markdown file in Drive via DriveApp: ${fileId}`);
        updated = true;
      } catch (errDriveApp) {
        console.warn(`DriveApp update failed for ${fileId}: ${errDriveApp.message}. Trying Drive.Files...`);
        try {
          if (typeof Drive !== 'undefined' && Drive.Files) {
            Drive.Files.update({}, fileId, blob);
            console.log(`Updated Markdown file in Drive via Drive.Files: ${fileId}`);
            updated = true;
          }
        } catch (errDriveAdvanced) {
          console.warn(`Drive.Files update failed for ${fileId}: ${errDriveAdvanced.message}`);
        }
      }
    }
    ...
    if (!updated) {
      ...
      if (existingFiles.hasNext()) {
        ...
        file.setContent(mdContent);
        console.log(`Updated existing Markdown file by name in folder: ${file.getId()}`);
      } else {
        const file = folder.createFile(fileName, mdContent, MimeType.PLAIN_TEXT);
        console.log(`CREATED NEW FILE! Update config: ${file.getId()}`);
      }
    }
  } catch (e) {
...
function loadValidTaxonomyPaths(ss) {
  const taxonomySheet = ss.getSheets().find(s => s.getSheetId().toString() === SYSTEM_CONFIG.SHEETS.LOS_TAXONOMY);
...
function loadAllowedAliases(ss) {
  const aliasSheet = ss.getSheets().find(s => s.getSheetId().toString() === SYSTEM_CONFIG.SHEETS.ALIAS_WHITELIST);
  let allowedAliases = [];
  if (aliasSheet) {
...
/**
 * Helper to extract the milestone name from a task title, its notes, or its parent task.
 * 
 * @param {string} title Task title.
 * @param {string} notes Task notes.
 * @param {string} [parentTitle] Parent task title (if task is a subtask).
 * @returns {string} Milestone name or "None".
 */
function extractMilestone(title, notes, parentTitle) {
  if (title && (/^\[milestone\]/i.test(title) || /^milestone:/i.test(title))) {
    return "Milestone";
  }
  if (parentTitle && (/^\[milestone\]/i.test(parentTitle) || /^milestone:/i.test(parentTitle))) {
    return parentTitle.replace(/^\[Milestone\]\s*/i, "").replace(/^Milestone:\s*/i, "").trim();
  }
  const match = (notes || "").match(/^Milestone:\s*(.*)$/mi);
  if (match && match[1].trim() && match[1].trim().toLowerCase() !== "none") {
    return match[1].trim().replace(/^\[Milestone\]\s*/i, "").trim();
  }
  try {
    const metaSplit = (notes || "").split('---SYSTEM_METADATA---');
    if (metaSplit.length > 1) {
      const parsed = JSON.parse(metaSplit[1].trim());
      if (parsed.milestone && parsed.milestone.toLowerCase() !== "none") {
        return parsed.milestone.replace(/^\[Milestone\]\s*/i, "").trim();
      }
    }
  } catch(e) {}
  return "None";
...
// ==========================================
// SECTION 2: AGENT TASK CLI READERS
// ==========================================
...
function dumpRecentEmailTasks() {
  const ss = getMasterSpreadsheet();
  const logGid = IS_CE_ENV ? "2131515996" : "967747913";
