...
# 1. Fetch config to get Gemini API Key
with open('src/Code_Config.js', 'r') as f:
    config_text = f.read()

# Very hacky extraction of the key just for this quick test script
import re
key_match = re.search(r'GEMINI_API_KEY:\s*"([^"]+)"', config_text)
if not key_match:
